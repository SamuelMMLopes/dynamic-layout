...
export class Occupancies {
  ...
  nextSlot({ widthOfVisibleArea, filledColumns, filledRows }: Occupancies.NextSlotInput): Occupancies.NextSlotOutput {
    const visibleColumns = Math.floor(widthOfVisibleArea / this.sliceWidth)
    let startRow = 1
    while (true) {
